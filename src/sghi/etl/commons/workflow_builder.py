...
class WorkflowBuilder(Generic[_RDT, _PDT]):
    ...
    :class:`sinks<sghi.etl.core.Sinks>`. This builder class offers a convenient
    ...
    def __init__(
        ...
        self._source_factories.extend(self._source_factories or ())
        ensure_optional_instance_of(
    ...
    def __call__(self) -> WorkflowDefinition[_RDT, _PDT]:
        ...
        :raise RuntimeError: If no ``Source`` has been provided. At least one
            ``Source`` is required.
    ...
    def composite_processor_factory(
        ...
        :param __composite_processor_factory: A factory function that accepts a
            ...
            MUST be a valid callable object. This MUST be a valid callable
            object.
    ...
    def composite_sink_factory(
        ...
        :param __composite_sink_factory: A factory function that accepts a
            ...
            callable object. This MUST be a valid callable object.
    ...
    def composite_source_factory(
        ...
        :param __composite_source_factory: A factory function that accepts a
            ...
            a valid callable object. This MUST be a valid callable object.
    ...
    def apply_processor(
        ...
        .. important::
            ...
            only supplies one ``Processor`` instance. As a result, the builder
            SHOULD NOT be used to construct multiple ``WorkflowDefinition``\ s.
            Doing so might cause errors when executing the assembled
            ``WorkflowDefinition``\ s, even though no errors will be raised
            when the :meth:`build` method is invoked multiple times on the
    ...
    def drain_to(self, sink: Sink[Any] | _SinkFactory[Any]) -> Self:
        ...
        .. important::
            ...
            supplies one ``Sink`` instance. As a result, the builder SHOULD NOT
            be used to construct multiple ``WorkflowDefinition``\ s.
            Doing so might cause errors when executing the assembled
            ``WorkflowDefinition``\ s, even though no errors will be raised
            when the :meth:`build` method is invoked multiple times on the
            builder. Clients of this class should be aware of this limitation.
    ...
    def draw_from(self, source: Source[Any] | _SourceFactory[Any]) -> Self:
        ...
        .. important::
            ...
            function, it is internally converted into a factory function that
            only supplies one ``Source`` instance. As a result, the builder
            SHOULD NOT be used to construct multiple ``WorkflowDefinition``\ s.
            Doing so might cause errors when executing the assembled
            ``WorkflowDefinition``\ s, even though no errors will be raised
            when the :meth:`build` method is invoked multiple times on the
    ...
    def _build_source_factory(self) -> _SourceFactory[_RDT]:
        match self._source_factories:
            ...
            case _:
                _err_msg: str = (
                    "No sources available. At least once source MUST be "
                    "provided."
                )
